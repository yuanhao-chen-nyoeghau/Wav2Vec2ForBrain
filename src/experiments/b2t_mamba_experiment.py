...
from src.util.nn_helper import calc_seq_len, compute_ctc_loss, create_fully_connected
...
class B2TMambaModel(B2TModel):
    def __init__(self, config: B2TMambaArgsModel, tokenizer: PreTrainedTokenizer):
        super().__init__()
        self.config = config
        self.tokenizer = tokenizer
        self.model = MambaLMHeadModel(config, tokenizer)
...
class B2tMambaExperiment(B2TExperiment):
    ...
    def _create_model(self):
        ...
        model = B2TMambaModel(self.config, self.tokenizer)
        return model
